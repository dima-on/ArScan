from flask import Flask, render_template, request, jsonify
import WorkFile
import os


app = Flask(__name__)
def testIMG(img, inT, inD, offsetY, offset_Down, with_Down):
    app.config['SEND_FILE_MAX_AGE_DEFAULT'] = 0
    return WorkFile.resImage(img, inT, inD, offsetY, offset_Down, with_Down)


@app.route('/')
def index():
    # Вызовите функции из вашей программы
    WorkFile.BeginFun()
    return render_template('GUI/index.html')


@app.route('/lick', methods=['POST'])
def lick():
    # Обработка загрузки фото
    uploaded_file = request.files['photo']
    offset_Top = request.form.get('offset')
    offset_Down = request.form.get('offsetDown')
    with_Down = request.form.get('with')
    TopIndex = request.form.get('TopIndex')

    if uploaded_file.filename != '':
        # Сохраняем загруженное фото в папке uploads
        upload_folder = 'static/'
        os.makedirs(upload_folder, exist_ok=True)
        uploaded_file.save(os.path.join(upload_folder, uploaded_file.filename))

        path = upload_folder + uploaded_file.filename

        im = testIMG(path, int(TopIndex), 1, offset_Top, offset_Down, with_Down)


    return jsonify({'result_image': im})


@app.route('/StartProgram', methods=['POST'])
def StartAll():
    Top_Image_Path = WorkFile.Top_Image_Path
    return jsonify({'result_image': Top_Image_Path})
if __name__ == '__main__':
    app.run(host='0.0.0.0')
