...
def body_Detect(Index, frame, landmarks, offset_Y):
    ...
    #21/11
    distanceX = (shoulder_left[0] - shoulder_right[0]) * int(Top_With[Index])
    distanceY = (hip_left.y - shoulder_left[1]) * int(Top_Height[Index])
...
def leg_Detect(Index, frame, landmarks, offset_Y, withDown):
    ...
    distanceX = int((hip_left[0] - hip_right[0]) * int(withDown) * 1.2)
    distanceY = (Heel_LeftY - hip_left[0]) * int(Down_Height[Index])
